...
- Issue 255 reran the harvest-aware factorial and promotion gate after runtime-complete harvest state/writeback landed on `main`; the current research family now resolves to `dekoning_fds + vegetative_unit_pruning + dekoning_fds`, but the KNU window still shows weak family discrimination, proxy-heavy research-family runtime state, and no promotion-grade holdout improvement
