...
from stomatal_optimiaztion.domains.tomato.tomics.alloc.interface import PipelineModel
...
class TomatoLegacyModelProtocol(Protocol):
    ...
    partition_policy_params: Mapping[str, object]
    start_date: datetime
    current_date: date
...
class TomatoLegacyAdapter:
    ...
    def __post_init__(self) -> None:
        ...
        self.model.partition_policy_params = (
            {
                str(key): value
                for key, value in self.partition_policy_params.items()
            }
            if isinstance(self.partition_policy_params, Mapping)
            else {}
        )
        self.reset_state()
