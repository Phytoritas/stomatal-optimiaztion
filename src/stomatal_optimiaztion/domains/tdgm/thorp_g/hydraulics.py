...
def e_from_soil_to_root_collar(
    *,
    ...
    r_r_h_min = params.beta_r_h / c_r_h
    r_r_h_min = np.where(c_r_h == 0, np.inf, r_r_h_min)

    r_r_v = params.beta_r_v * dz**2 / c_r_v
    r_r_v = np.where(c_r_v == 0, np.inf, r_r_v)
    ...
    for i in range(psi_soil_by_layer.size):
        if c_r[i] > 0:
            psi_src_min = float(min(float(psi_soil_by_layer[i]), psi_rc))
            psi_src_max = float(max(float(psi_soil_by_layer[i]), psi_rc))

            if psi_rc == float(psi_soil_by_layer[i]):
                f_ri = float(params.vc_r(psi_src_min))
                if psi_src_min > 0:
                    f_ri = float(params.vc_r(0.0))
                r_r_h = _safe_div(float(r_r_h_min[i]), f_ri)
                r_r = r_r_h + float(r_r_v_sum[i])
                e_i = -(params.rho * params.g * float(z_soil_mid[i]) / 1e6) / r_r / la
            elif (float(psi_soil_by_layer[i]) - psi_rc) == (params.rho * params.g * float(z_soil_mid[i]) / 1e6):
                e_i = 0.0
                ...
                r_r = r_r_h + float(r_r_v_sum[i])
                e_i = (
                    float(psi_soil_by_layer[i]) - psi_rc - params.rho * params.g * float(z_soil_mid[i]) / 1e6
                ) / r_r / la
            ...
            e_soil[i] = 0.0
            f_r[i] = float(params.vc_r(float(psi_soil_by_layer[i])))
...
def stomata(
    *,
    ...
    k_sw_max = params.b2 * (d / params.d_ref) ** params.c2 * (1 - (d_hw / d) ** (params.c2 / params.c0 + 1))
    dk_sw_max_d_d = (
        k_sw_max
        / d
        * (params.c2 + (params.c2 / params.c0 + 1 - params.c2) * (d_hw / d) ** (params.c2 / params.c0 + 1))
    ...
    psi_rc_3_0 = np.array([psi_rc_ub_0, 0.5 * (psi_rc_ub_0 + psi_rc_lb_0), psi_rc_lb_0], dtype=float)
    ...
    psi_rc_3_crit = np.array([psi_rc_0, 0.5 * (psi_l_crit + psi_rc_0), psi_l_crit], dtype=float)
    psi_l_3_crit = np.full(3, np.nan, dtype=float)

    for i in range(3):
        psi_rc_i = float(psi_rc_3_crit[i])
    ...
    while float(np.max(psi_rc_3_crit) - np.min(psi_rc_3_crit)) > 1e-3:
        ...
        psi_rc_3_crit = np.array([psi_rc_ub_crit, psi_rc_m_crit, psi_rc_lb_crit], dtype=float)
    ...
    for i, psi_rc_i in enumerate(psi_rc_curve):
        ...
        e_curve[i] = e_i
        e_soil_curve[:, i] = res_uptake.e_soil
        r_r_h_curve[:, i] = res_uptake.r_r_h
        r_r_v_curve[:, i] = res_uptake.r_r_v
        f_r_curve[:, i] = res_uptake.f_r
        ...
        psi_s_curve[i] = psi_s_i

        psi_l_i = psi_s_i
        for _ in range(n):
            vc = float(params.vc_l(min(0.0, psi_l_i)))
            psi_l_i = psi_l_i - _safe_div(e_i, (n * params.k_l * vc))
        psi_l_curve[i] = psi_l_i
    ...
    j = (j_max + j_phi - np.sqrt((j_max + j_phi) ** 2 - 4 * params.c_prime2 * j_max * j_phi)) / (2 * params.c_prime2)
    ...
    if np.any(inf_mask):
        a_c_rate[inf_mask] = a_c[inf_mask] * (params.c_a - gamma_star[inf_mask]) / (params.c_a + b_c[inf_mask]) - r_d_curve[inf_mask]
        a_j_rate[inf_mask] = a_j[inf_mask] * (params.c_a - gamma_star[inf_mask]) / (params.c_a + b_j[inf_mask]) - r_d_curve[inf_mask]

    a_n_curve = (a_c_rate + a_j_rate - np.sqrt((a_c_rate + a_j_rate) ** 2 - 4 * params.c_prime1 * a_c_rate * a_j_rate)) / (
        2 * params.c_prime1
    )

    d_a_n_d_a_c = (1 - ((a_c_rate + (1 - 2 * params.c_prime1) * a_j_rate) / np.sqrt((a_c_rate + a_j_rate) ** 2 - 4 * params.c_prime1 * a_c_rate * a_j_rate))) / (
        2 * params.c_prime1
    )
    d_a_n_d_a_j = (1 - ((a_j_rate + (1 - 2 * params.c_prime1) * a_c_rate) / np.sqrt((a_c_rate + a_j_rate) ** 2 - 4 * params.c_prime1 * a_c_rate * a_j_rate))) / (
        2 * params.c_prime1
    )

    var_h = (j / 4 + g_c_curve * (params.c_a + 2 * gamma_star) - r_d_curve) / 2
    d_a_j_d_j = (1 / 8) * (g_c_curve * (params.c_a - gamma_star)) / (var_h - a_j_rate)
    d_a_j_d_j = np.where(inf_mask, 1 / 4 * (params.c_a - gamma_star) / (params.c_a + b_j), d_a_j_d_j)

    d_j_d_j_phi = (1 - ((j_phi + (1 - 2 * params.c_prime2) * j_max) / np.sqrt((j_max + j_phi) ** 2 - 4 * params.c_prime2 * j_max * j_phi))) / (
        2 * params.c_prime2
    )
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        ...
        f_sw = la * e_curve / (psi_rc_curve - psi_s_curve - params.rho * params.g * h / 1e6) / k_sw_max
        f_sw[0] = float(
            np.mean(
                np.minimum(
                    1.0, params.vc_sw(np.linspace(float(psi_rc_curve[0]), float(psi_s_curve[0]), 10))
                )
            )
        )
    ...
    d_p = 1e-2
    d_vc_r_d_psi_rc = (params.vc_r(np.minimum(0.0, psi_rc_curve + d_p)) - params.vc_r(np.minimum(0.0, psi_rc_curve))) / d_p
    d_vc_r_d_psi_soil = (
    ...
    d2_vc_r_d_psi_soil2 = np.repeat(d2_vc_r_d_psi_soil2[:, None], psi_rc_curve.size, axis=1)

    df_r_d_psi_rc = (f_r_curve - params.vc_r(np.minimum(0.0, psi_rc_curve))) / (
    ...
    d2f_r_d_psi_rc2 = np.where(close_mask2, (1 / 3) * d2_vc_r_d_psi_soil2, d2f_r_d_psi_rc2)

    df_sw_d_psi_s = (f_sw - params.vc_sw(np.minimum(0.0, psi_s_curve))) / (psi_rc_curve - psi_s_curve)
    df_sw_d_psi_rc = (params.vc_sw(np.minimum(0.0, psi_rc_curve)) - f_sw) / (psi_rc_curve - psi_s_curve)
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        ...
        c_var = np.sum((1 / la - e_soil_curve * r_r_h_curve / f_r_curve * df_r_d_psi_rc) / r_r, axis=0)
        d_var = params.vc_l(np.minimum(0.0, psi_s_curve)) / f_l
        e_var = params.vc_l(np.minimum(0.0, psi_l_curve)) / f_l

        k_canopy_curve = e_var / (1 / f_l / params.k_l + d_var / a_var * (b_var / c_var + la / f_sw / k_sw_max))
    ...
    d_psi_rc0_d_c_r_v = 1 / c0_var / c_r_v * _reverse_cumsum(e_soil0 * r_r_v0 / r_r0, axis=0)

    k_canopy_max = 1 / (1 / params.k_l / f_l0 + 1 / c0_var + la / f_sw0 / k_sw_max)
    if k_canopy_max < 0:
        raise RuntimeError("Negative k_cmax")

    df_l0_d_psi_rc0 = (params.vc_l(min(0.0, psi_l0 + d_p)) - params.vc_l(min(0.0, psi_l0))) / d_p
    df_l0_d_h = -params.rho * params.g / 1e6 * float(df_l0_d_psi_rc0)
    df_sw0_d_psi_rc0 = (params.vc_sw(min(0.0, psi_rc0)) - params.vc_sw(min(0.0, psi_l0))) / (
        params.rho * params.g * h / 1e6
    )
    df_sw0_d_h = (params.vc_sw(min(0.0, psi_l0)) - f_sw0) / h

    dk_canopy_max_d_d = k_canopy_max**2 * (
        (df_l0_d_h / params.k_l / (f_l0**2) + la * df_sw0_d_h / k_sw_max / (f_sw0**2)) * d_h_d_d
        + la / (k_sw_max**2) / f_sw0 * dk_sw_max_d_d
    )
    dk_canopy_max_d_la = k_canopy_max**2 * (1 / k_sw_max / f_sw0 - 1 / (c0_var**2 * la**2) * float(np.sum(1 / r_r0)))
    ...
    dk_canopy_max_d_c_r_h = k_canopy_max**2 * (
        (
            ...
            * float(
                np.sum(
                    (
                        (2 * e_soil0 / f_r0**2 * r_r_h0 / r_r0 * df_r0_d_psi_rc0)
                        + (
                            r_r_h0
                            / f_r0
                            / r_r0**2
                            * (1 / la - e_soil0 * r_r_h0 / f_r0 * df_r0_d_psi_rc0)
                        )
                    )
                    * df_r0_d_psi_rc0
                    - e_soil0 / f_r0 * r_r_h0 / r_r0 * d2f_r0_d_psi_rc0
                )
            )
        )
        * d_psi_rc0_d_c_r_h
    ...
    dk_canopy_max_d_c_r_v = k_canopy_max**2 * (
        ...
        * d_psi_rc0_d_c_r_v
        ...
        / (c0_var**2)
        / c_r_v
        * _reverse_cumsum(r_r_v0 / (r_r0**2) * (-e_soil0 * r_r_h0 * df_r0_d_psi_rc0 / f_r0 + 1 / la), axis=0)
    )
    ...
    if a_n_max > 0:
        ...
        # NOTE: For extremely dry states, curves can contain +/-inf and cause
        # `np.diff` to warn on inf-inf subtraction; MATLAB does not surface this
    ...
    g_var = (
        ...
        * a_n
        / (a_n_max**2)
        * (psi_l0 - psi_l)
        / (1 + lambda_wue_crit * k_canopy_max * a_n / (a_n_max**2) * (psi_l - psi_lcrit))
    )
    h_var = k_canopy_max / k_canopy * a_n / a_n_max - g_var * (1 + k_canopy_max / k_canopy * (1 - a_n / a_n_max))
    i_var = e / k_canopy_max + a_n / a_n_max * (psi_l0 - psi_l) - g_var * (
        (e_max - e) / k_canopy_max + a_n / a_n_max * (psi_l - psi_lcrit)
    )

    denom = (
        1 / d_opt * (1 / f_l_opt / params.k_l + e_opt / k_canopy * (1 + g_var) / (h_var - 1))
        + 1 / a_opt * (b_opt / c_opt + la / f_sw_opt / k_sw_max)
    )

    d_e_d_d = (
        (
            -(1 / a_opt + e_opt / d_opt * k_canopy_max / k_canopy / (h_var - 1)) * params.rho * params.g / 1e6 * d_h_d_d
            + e_opt
            / d_opt
            / (h_var - 1)
            / k_canopy
    ...
    d_e_d_la = (
        (
            ...
            / (h_var - 1)
            / k_canopy
            * (i_var * dk_canopy_max_d_la - g_var * d_a_n_d_r_abs_crit / lambda_wue_crit * d_r_abs_d_la)
        )
        / denom
    )
    ...
    d_e_d_c_r_h = (
        (
            e_opt / d_opt / (h_var - 1) / k_canopy * (i_var * dk_canopy_max_d_c_r_h + k_canopy_max * d_psi_rc0_d_c_r_h)
    ...
    d_e_d_c_r_v = (
        (
            e_opt / d_opt / (h_var - 1) / k_canopy * (i_var * dk_canopy_max_d_c_r_v + k_canopy_max * d_psi_rc0_d_c_r_v)
            + b_opt / a_opt / c_opt / c_r_v * _reverse_cumsum(e_soil * r_r_v_opt / r_r_opt, axis=0)
        )
        / denom
    )
