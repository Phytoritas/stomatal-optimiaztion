...
_TDGM_PACKAGE = "stomatal_optimiaztion.domains.tdgm"
...
def model_card_dir() -> Traversable:
    return files(_TDGM_PACKAGE).joinpath("model_card")
...
def require_equation_ids(eq_ids: Iterable[str]) -> None:
    ...
    if missing:
        raise KeyError(f"Missing equation ids in TDGM model_card: {missing}")
