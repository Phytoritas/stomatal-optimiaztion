...
_THORP_PACKAGE = "stomatal_optimiaztion.domains.thorp"
...
def model_card_dir() -> Traversable:
    return files(_THORP_PACKAGE).joinpath("model_card")
...
def require_equation_ids(eq_ids: Iterable[str]) -> None:
    ...
    if missing:
        raise KeyError(f"Missing equation ids in THORP model_card: {missing}")
