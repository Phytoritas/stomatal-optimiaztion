...
- Current phase: the recursive architecture refactoring wave is closed, including the former root `TDGM` later-horizon control gap `D-108`; the architecture spine and validation contract are green, continuous root `TDGM` parity is exact through day `784.5`, the shipped post-day-`791.5` control reopening is now recorded as a reference-payload resume artifact in `docs/architecture/review/tdgm-reference-payload-resume-provenance-note.md`, the TOMICS naming and first hybrid tomato partition-policy slice is complete through issue `#227` / module `114`, the TOMICS-Alloc research-only architecture pipeline is complete through issue `#231` / module `115`, issue `#233` / module `116` standardizes reusable repo-local graph rendering on Plotkit spec bundles with the live TOMICS figure scripts migrated onto that contract under a PNG-only bundle policy, and issue `#236` / module `117` now adds actual KNU greenhouse validation, greenhouse substrate proxying, and current-vs-promoted TOMICS allocation promotion scoring without changing the shipped `tomics` default
...
7. use issue `#236` / module `117` artifacts under `out/tomics_current_factorial_knu/`, `out/tomics_promoted_factorial_knu/`, and `out/tomics_current_vs_promoted_knu/` as the current actual-data promotion baseline for any later shipped-default proposal
