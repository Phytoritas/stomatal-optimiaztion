...
- Current phase: the recursive architecture refactoring wave is closed, including the former root `TDGM` later-horizon control gap `D-108`; the architecture spine and validation contract are green, continuous root `TDGM` parity is exact through day `784.5`, the shipped post-day-`791.5` control reopening is now recorded as a reference-payload resume artifact in `docs/architecture/review/tdgm-reference-payload-resume-provenance-note.md`, the TOMICS naming and first hybrid tomato partition-policy slice is complete through issue `#227` / module `114`, the TOMICS-Alloc research-only architecture pipeline is complete through issue `#231` / module `115`, issue `#233` / module `116` standardizes reusable repo-local graph rendering on Plotkit spec bundles with the live TOMICS figure scripts migrated onto that contract under a PNG-only bundle policy, issue `#236` / module `117` adds the actual KNU greenhouse current-vs-promoted baseline, issue `#239` / module `118` completes the fair-validation pipeline with harvested-yield observation semantics, hidden-state reconstruction, root-zone inversion, equal-budget calibration, and the promotion gate that keeps shipped `tomics` incumbent, issue `#243` / module `119` promotes harvest into a first-class TOMICS architecture family without changing the incumbent shipped baseline, issue `#247` closes the external-harvest zero-yield replay seam, issue `#249` cleans harvested-vs-total observation semantics and adds post-writeback dropped-mass guardrails, and issue `#251` reruns the harvest-aware factorial and promotion gate while still keeping shipped TOMICS plus incumbent TOMSIM harvest as the public baseline
...
7. use issue `#239` / module `118` artifacts under `out/tomics/validation/knu/fairness/observation-eval/`, `out/tomics/validation/knu/fairness/state-reconstruction/`, `out/tomics/validation/knu/fairness/rootzone-reconstruction/`, `out/tomics/validation/knu/fairness/calibration/`, and `out/tomics/validation/knu/fairness/promotion-gate/` as the current fair-validation promotion baseline for any later shipped-default proposal
8. use issues `#243`, `#247`, `#249`, and `#251` artifacts under `out/tomics_knu_harvest_family_factorial/` and `out/tomics_knu_harvest_promotion_gate/` when harvest-family effects must be separated from allocator-family effects, and rerun `scripts/run_tomics_knu_harvest_family_factorial.py` plus `scripts/run_tomics_knu_harvest_promotion_gate.py` after any future harvest replay, writeback, or observation-operator change
9. audit `src/stomatal_optimiaztion/domains/tomato/tomics/alloc/validation/harvest_family_eval.py`, `harvest_family_summary.py`, `harvest_mass_balance_eval.py`, and `harvest_calibration_bridge.py` whenever the harvest-aware score surface or promotion guardrails change
