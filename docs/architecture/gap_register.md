...
Current open gaps:
- `D-108`: root `TDGM` canonical full-series control rerun still reopens against the legacy MATLAB payload after day `791.5` even though slice `109` fixed the first proven seam near day `497.5` and the fast bounded parity tests pass
