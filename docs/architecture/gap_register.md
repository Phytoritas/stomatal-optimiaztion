...
Current open gaps:
- `D-108`: root `TDGM` canonical full-series control rerun still reopens against the legacy MATLAB payload after day `791.5`; module `110` narrows the next likely culprit to the `THORP-G stomata/hydraulics sensitivity -> daily optimal allocation` seam
