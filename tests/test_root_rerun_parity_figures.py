...
def test_render_thorp_rerun_parity_bundle_writes_outputs(tmp_path: Path) -> None:
    ...
    assert metadata["diff_summary"]["height"]["max_abs_diff"] == pytest.approx(0.0)
    assert metadata["diff_summary"]["assimilation"]["max_abs_diff"] == pytest.approx(0.0)
...
def test_render_gosm_rerun_parity_suite_writes_outputs(tmp_path: Path) -> None:
    ...
    rh_metadata = json.loads(Path(summary["rh"]["metadata"]).read_text(encoding="utf-8"))
    control_metadata = json.loads(Path(summary["control"]["metadata"]).read_text(encoding="utf-8"))

    assert Path(summary["control"]["png"]).exists()
    assert Path(summary["p_soil_min_true"]["pdf"]).exists()
    assert summary["p_soil_min_imag"] is None
    assert control_metadata["comparison"]["g_c_vec"]["max_abs_diff"] == pytest.approx(0.0)
    assert rh_metadata["diff_summary"]["g_c_steady_state"] < 1e-8
...
def test_render_tdgm_rerun_parity_suite_can_filter_cases(tmp_path: Path) -> None:
    ...
    metadata = json.loads(Path(summary["thorp_data_control_turgor"]["metadata"]).read_text(encoding="utf-8"))
...
def test_render_root_rerun_parity_figures_script_entrypoint(tmp_path: Path) -> None:
    ...
    assert Path(summary["gosm"]["control"]["metadata"]).exists()
    assert Path(summary["tdgm"]["thorp_data_control_turgor"]["pdf"]).exists()
