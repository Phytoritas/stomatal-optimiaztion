...
def test_factorial_runner_writes_design_and_summary_artifacts(tmp_path: Path) -> None:
    ...
    assert summary_plot.with_suffix(".pdf").exists()
    assert (summary_plot.parent / "summary_plot_data.csv").exists()
    assert (summary_plot.parent / "summary_plot_spec.yaml").exists()
    assert (summary_plot.parent / "summary_plot_resolved_spec.yaml").exists()
    assert (summary_plot.parent / "summary_plot_tokens.yaml").exists()
    assert (summary_plot.parent / "summary_plot_metadata.json").exists()
    assert Path(summary["summary_plot_pdf"]).exists()
    assert Path(summary["summary_plot_metadata"]).exists()
